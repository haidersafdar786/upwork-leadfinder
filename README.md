...
Upwho finds the public identity behind anonymized Upwork jobs. It groups jobs by buyer, keeps the evidence used for each match, and links verified public websites and contact paths.

It runs locally for one freelancer. The dashboard listens on `127.0.0.1` and runs are saved under `runs/`. Upwho is not affiliated with Upwork or OpenCode. Review possible matches and contact details before using them.
...
3. OpenCode analysts and verifiers compare the collected text. Upwho keeps a claim only when it has exact supporting evidence and labels the result `verified`, `possible`, or `unknown`.
...
Upwho uses Chrome DevTools Protocol to connect to the browser. If no endpoint is available at `http://127.0.0.1:9222`, it tries to start an installed Chromium-family browser. If your browser is already open without remote debugging, fully quit it and run Upwho again. Set `UPWHO_CDP_URL` to use another endpoint.
...
Search queries can also be complete Upwork search URLs. Existing job IDs are skipped across runs; add `--force` to process them again. Add `--no-model` for collection-only runs without identity or web enrichment.

## Output

Each run is stored in `runs/<timestamp>_<feed>/`:
